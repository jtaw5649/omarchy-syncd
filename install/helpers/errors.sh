...
catch_errors() {
	...
	if [[ "${OMARCHY_SYNCD_SUPPRESS_FAILURE_UI:-0}" == "1" ]]; then
		unset OMARCHY_SYNCD_SUPPRESS_FAILURE_UI
		exit "$exit_code"
	fi
	...
	if [[ -n "${OMARCHY_SYNCD_FAILURE_REASON:-}" ]]; then
		gum style "Failure reason:"
		while IFS= read -r line; do
			gum style "$line"
		done <<<"$OMARCHY_SYNCD_FAILURE_REASON"
	fi
